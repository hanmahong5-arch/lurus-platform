...
Image tags follow `main-<sha7>` for production and `latest` for staging
(immutable-tag policy from Track L RL2.2). To pin back to a known-good
build:

```bash
# 1. Identify the last good SHA from git log or ArgoCD history.
# 2. Update the Kustomize image tag (never use kubectl patch for
#    permanent changes — ArgoCD auto-sync will overwrite it).
cd deploy/k8s/overlays/full
kustomize edit set image ghcr.io/hanmahong5-arch/lurus-platform-core=ghcr.io/hanmahong5-arch/lurus-platform-core:main-abc1234
git commit -am "rollback: pin platform-core to abc1234"
git push
...
Never force-push to `main`. Never `kubectl set image` — the change will
evaporate on the next auto-sync.
